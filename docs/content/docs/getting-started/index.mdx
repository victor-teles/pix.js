---
title: Introduction
description: What is Pix.js and how to use it in your project
---
<img
  className="only-on-light"
  src="https://raw.githubusercontent.com/victor-teles/pix.js/refs/heads/main/.github/banner.webp"
  alt="Pix.js logo"
/>
<img
  className="only-on-dark"
  src="https://raw.githubusercontent.com/victor-teles/pix.js/refs/heads/main/.github/banner-dark.webp"
  alt="Pix.js logo"
/>

## What is Pix.js?

Pix.js is a set of tools for integrating PIX payments into your JavaScript/TypeScript projects.
It has different packages:

<Cards>

<Card icon={<CpuIcon className="text-purple-300" />} title='@pix.js/core'>
Core logic and components for PIX validations and QR code generation.
</Card>

<Card icon={<QrCodeIcon className="text-purple-300" />} title='@pix.js/qrcode' href='/en/docs/getting-started/qrcode'>
QR code factory and parser for both static and dynamic PIX QR codes.
</Card>

<Card icon={<CheckCheckIcon className="text-purple-300" />} title='@pix.js/validator' href='/en/docs/getting-started/validators'>
Validator for PIX keys, CRC checksums, and other PIX-related data.
</Card>

<Card icon={<BookAIcon className="text-purple-300" />} title='@pix.js/dict' href='/en/docs/getting-started/dict'>
DICT API SDK for working with Brazil's DICT (Diretório de identificadores de contas transacionais) API.
</Card>

</Cards>


